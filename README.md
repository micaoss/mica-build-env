...
| Row | Image | Adds |
| --- | --- | --- |
| `image base` | base, on Debian trixie | git, file, binutils, xz, curl, wget, openssl, jq, dpkg-dev, mmdebstrap, python3 (also as `python`), bun |
| `image c` | c, on base | build-essential, cmake, pkgconf, autotools, libtool, ccache |
| `image go` | go, on c | Go, cgo |
| `image rust` | rust, on c | rustc, cargo, cross std and linker, clippy, rustfmt, cargo-nextest, cargo-deny, dbus-daemon |

`locks/upstream.lock` pins every third-party input: the upstream images Mica
OS uses (the Debian base, the Dockerfile frontend, CI tools), mirrored whole
into the same package as `upstream.<path>.<tag>.<digest12>` keeping the
upstream digest, and the toolchain archives by sha256. These images build from
those mirrors. `params.env` holds the build parameters that are not pins:
...
which builds from the tag: its mirror job copies the images of `locks/upstream.lock`
not mirrored yet, its plan job lists the images these inputs do not have
...
the release row and the `image` rows of the four images and of every mirror)
...
bash tests/publish-test.sh                        # the lock vectors, which inputs move which image, the mirrors, every release refusal
